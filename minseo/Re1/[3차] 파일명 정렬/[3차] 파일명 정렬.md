...
## 아이디어
- 그룹을 3개로 나눈다
    1. ([^0-9]+) : 하나 이상 연속된 문자를 찾는데 이는 숫자가 아닌 문자를 의미
    2. ([0-9]{1,10}) : 최소1자리 최대 10자리의 숫자를 찾음
    3. (.*) 0개 이상의 임의의 문자 , 마지막 $은 문자열의 끝을 나타냄
- 파일 이름을 정규표현식으로 분해함 여기서 a (숫자가 아닌 부분) b(숫자 부분) 을 나타냄
- Head 부분을 비교함 toLowerCase() 이용해 Head를 소문자로 변환한 후 비교
- 만약 첫번째 인자 a가 두번째 인자 b보다 앞에 있어야 한다고 판단하면 return -1을 해주는데 이는 a를 b 앞으로 정렬한다 return 1 을 하면 a가 b보다 뒤에 있어야 한다 판단하여 a를 b뒤로 정렬한다
- 후에 숫자값도 정수로 변환하여 오름차순 정렬한다
