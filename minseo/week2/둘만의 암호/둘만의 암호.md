...
>
> ### 아이디어
> skip에 있는 알파벳은 s문자열에서 빼야하므로 skip에 없는 알파벳만 추가하는 로직을 만들어야함
>skip 알파벳을 제외한 새로운 알파벳 문자열에서 각 문자의 index 만큼 뒤로 이동시켜줘야하므로 이건 간단한 for 반복문을 만들어서 실행
> -> 범위를 초과할 수도 있으므로 이에 대한 코드도 추가해야함
