...
### 아이디어
1. 정현이가 할인 행사하는 10일동안 자신이 원하는 물건과 그 물건의 개수가 10일 연속으로 일치해야하므로 slice 를 통해 discount 배열을 10칸씩 나누자
2. 만약 원하는 물건과 개수가 10일간에 포함되어있다면 result를 증가시켜준다. 이 문제에서 순서는 상관없으므로, 순서관계는 생각하지 않는다.
