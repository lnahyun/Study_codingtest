...
## 아이디어
-> 최소공배수를 구한ㄴ 방법에는 1. 공약수로 나누기, 2. 지수이용나누기가 있다. 이번 풀이에선 최대공약수를 구하는 함수를 구현후, 최소 공배수를 구현하였다.
solution 함수에서는 현재까지의 최소공배수와 배열의 다음 숫자를 비교해 새로운 최소공배수를 계산하는 방식으로 작동?하며, 배열의 모든 숫자를 처리하면서 전체 숫자의 최소공배수를 반환한다
