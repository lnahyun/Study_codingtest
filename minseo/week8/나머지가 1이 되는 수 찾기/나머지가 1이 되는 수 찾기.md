...
>
> ## 아이디어
> - 처음에 너무 복잡하게 생각해서 n-1 = x , x값에 루트 씌우고 정수이면 루트씌운 값이 답이고.. 정수이면 x가 답이다.. 이런식으로 풀었는데 기본 테스트 케이스 1,2 는 통과했지만 제출하니 다른 케이스들이 오답이 많이떠서 좀 다시 생각해보고 그냥 있는 그대로 x 는 무조건 1보단 크니까 x > 2 라하고, n % x === 1 이면 return x라 하니 바로 정답 정말 간단히 푸는 문제였음
