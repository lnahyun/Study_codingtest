...
### 아이디어
우선 거리를 측정해야 하므로, 키패드 숫자들을 좌표로 구분한다.<br>
- Node class를 만들어서 숫자의 x, y좌표를 표시
- HashMap에 숫자, 좌표를 넣어서 숫자로 좌표를 구분
ex) 1의 x 좌표는 0,0 이므로, Node 객체를 만들어서 0,0 을 넣는다.<br>
- 현재 손가락의 위치를 표시할 Node형의 배열을 만든다.(left=0, right=1)
- 현재 손가락의 위치 초기화시켜주기

**계산**
1. numbers에서 입력되는 숫자의 좌표를 map에서 꺼내온다.<br>
2. 왼쪽 키패드일 경우는, y좌표가 0일 때 이므로, 스트링빌더에 L을 넣고 현재 손가락 위치를 업데이트한다.
3. 오른쪽 키패드일 경우는, y좌표가 2일 때 이므로, 스트링빌더에 R을 넣고 현재 손가락 위치를 업데이트한다.
4. 중간 키패드일 경우에는, 가까운 손가락으로 눌러야 하므로, 절대값을 사용하여 거리를 계산한 후 조건을 체크해준다.
5. 만약 거리가 비슷할 경우엔 입력된 hand참고하여 append해준다.
