...
### 아이디어
Integer[] 배열 paint를 하나 더 만들어서 색칠된 벽을 1, 색칠되지 않은 벽을 0 으로 초기화시킨다.<br>
\* **Integer형**인 이유는? <br>
: 전체적인 로직을 보면, paint 배열에서 0인 부분을 찾고 그 부분부터 롤러의 길이만큼 색칠한다. <br>
여기서 indexOf메서드는 배열에서 사용할 수 없으므로 paint배열을 리스트로 변경한 후 indexOf를 사용해야하는데,<br>
int[]로 선언한다면 int[]가 전체 객체가 되어 리스트 안에 들어간다. -> 제대로 변환이 안 됨(자세한 내용은 week4의 자료구조.md를 참고)<br>
추가적으로, indexOf()를 사용했지만 그 값을 찾지 못하면 객체인 null이 반환되므로 start도 Integer형임<br><br>

위에서 설명했다시피,
1. paint배열에서 0인 부분(색칠되지 않은 부분)을 찾는다.
2. 그부분부터 롤러의 길이만큼 색칠한다.(벽을 벗어나지 않도록 처리)<br><br>

인데.. 이 문제는 좀 쉬운 편이라 얼렁뚱땅 코드짜도 됐는데 만약 색칠한 부분의 가장 첫 번째 벽을 리턴하라 하면 로직을 완전히 바꿔야할 것 같다.<br>
조건이 롤러의 길이(m)보다 벽의 길이(n)이 더 크기 때문에 색칠되지 않은 벽을 찾아 롤러의 길이만큼 for문을 돌리고 **돌리는 와중에 벽을 벗어났다? 그냥 break!!ㅋ** 해주면<br>
문제는 풀리지만 더 복잡한 조건에서는 풀리지 않았을듯.
