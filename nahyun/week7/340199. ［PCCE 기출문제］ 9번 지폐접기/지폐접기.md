...
### 아이디어
문제에 푸는 방법이 너무 잘 나와있어서.. 그대로 풀었다! <br>
간단하게 설명하자면, walletMax과 walletMin값을 설정한다.<br>
while문을 돌리며 지폐를 접는다. <br>
bill[0]이 가로, 세로중 더 큰 값일 때 wallet 안에 들어간다면 answer을 리턴해준다.(bill[1]이 큰 값일 때도 동일함) <br>
어느 조건문에도 걸리지 않으면 한 번 더 접어야 하는 것이므로 answer++ 해준다. <br>
